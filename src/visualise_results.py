...
def plot_results_with_errorbars(summary, dataset_name):
    ...
    labels = [f"{row['Model']}\n({row['Privacy Setting']})" for _, row in summary.iterrows()]
    x = np.arange(len(labels))

    fig, axes = plt.subplots(2, 2, figsize=(20, 13))
    fig.suptitle(f'{dataset_name} — Privacy vs Performance (mean ± std, 5 seeds)',
    ...
    for metric, ax, color in zip(metrics, axes.flatten(), colors):
        ...
        bars = ax.bar(x, means, color=color, alpha=0.8, edgecolor='white',
    ...
    plt.tight_layout()
    fname = f'{dataset_name.lower().replace("-", "_").replace(" ", "_")}_results.png'
...
def plot_dp_tradeoff(summary, dataset_name):
    ...
    dp_df['Epsilon'] = dp_df['Privacy Setting'].str.extract(r'e=(\d+\.?\d*)').astype(float)

    baseline = summary[summary['Privacy Setting'] == 'Baseline (No Privacy)']

    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle(f'{dataset_name} — DP Privacy-Utility Tradeoff (mean ± std)',
    ...
    for ax, metric in zip(axes, ['Accuracy', 'F1 Score']):
        for model_name, color in model_colors.items():
            model_data = dp_df[dp_df['Model'] == model_name].sort_values('Epsilon', ascending=False)
            if model_data.empty:
                continue
            ax.errorbar(
                model_data['Epsilon'],
        ...
        bl_colors = {'Logistic Regression': '#2196F3', 'Random Forest': '#4CAF50'}
        for _, row in baseline.iterrows():
            ax.axhline(y=row[f'{metric} mean'], linestyle='--', alpha=0.6,
                       color=bl_colors.get(row['Model'], 'gray'),
    ...
    plt.tight_layout()
    fname = f'{dataset_name.lower().replace("-", "_").replace(" ", "_")}_tradeoff.png'
    plt.savefig(GRAPHS_DIR / fname, dpi=150, bbox_inches='tight')
    plt.show()
    print(f"Saved: {fname}")
...
def plot_runtime(summary, dataset_name):
    ...
    fig.suptitle(f'{dataset_name} — Training Runtime Comparison (mean ± std)',
                 fontsize=14, fontweight='bold')

    bars = ax.bar(x, means, color='#607D8B', alpha=0.8, edgecolor='white',
                  linewidth=0.5, yerr=stds, capsize=3)
    ...
    plt.tight_layout()
    fname = f'{dataset_name.lower().replace("-", "_").replace(" ", "_")}_runtime.png'
    plt.savefig(GRAPHS_DIR / fname, dpi=150, bbox_inches='tight')
    plt.show()
    print(f"Saved: {fname}")


nhanes = load_summary(RESULTS_DIR / 'nhanes_results_summary.csv')
covid  = load_summary(RESULTS_DIR / 'covid_results_summary.csv')
